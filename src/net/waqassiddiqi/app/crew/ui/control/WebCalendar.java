...
public class WebCalendar extends WebPanel {
	 ...
	 * Used icons.
	 */
	public static final ImageIcon previousSkipIcon = com.alee.extended.date.WebCalendar.previousSkipIcon;
	public static final ImageIcon previousIcon = com.alee.extended.date.WebCalendar.previousIcon;
	public static final ImageIcon nextIcon = com.alee.extended.date.WebCalendar.nextIcon;
	public static final ImageIcon nextSkipIcon = com.alee.extended.date.WebCalendar.nextSkipIcon;

	/**
	 * Date selection listeners.
	 */
	protected List<DateSelectionListener> dateSelectionListeners = new ArrayList<DateSelectionListener>(
			1);
	...
	/**
	 * Whether should perform horizontal slide animation or not. todo Replace
	 * with transition panel customizer
	 */
	...
	/**
	 * Previously displayed month date. Used to perform animation in a specific
	 * direction.
	 */
	...
	public WebCalendar(final Date date, final Date crewSignOnDate, final Date maxEnabledDate) {
		...
		previousSkip = WebButton.createIconWebButton(previousSkipIcon,
		...
		previous = WebButton.createIconWebButton(previousIcon,
				StyleConstants.smallRound, true);
		...
		next = WebButton.createIconWebButton(nextIcon,
				StyleConstants.smallRound, true);
		...
		nextSkip = WebButton.createIconWebButton(nextSkipIcon,
				StyleConstants.smallRound, true);
		...
		weekHeaders.setMargin(StyleConstants.shadeWidth,
				StyleConstants.shadeWidth - 1, StyleConstants.shadeWidth + 1,
	...
	/**
	 * Switches to new title label. In case animation is disabled simply changes
	 * title text.
	 */
	...
	protected WebPanel createMonthPanel() {
		...
		monthDays.setMargin(StyleConstants.shadeWidth - 1,
				StyleConstants.shadeWidth - 1, StyleConstants.shadeWidth - 1,
				StyleConstants.shadeWidth - 1);
	...
	protected void switchMonth(final boolean animate) {
		if (animate) {
			...
			effect.setDirection(oldShownDate.getTime() > shownDate.getTime() ? getNextDirection(ltr)
					: getPrevDirection(ltr));
			effect.setSpeed(20);
	...
	protected Direction getNextDirection(final boolean ltr) {
		return horizontalSlide ? ltr ? Direction.right : Direction.left
				: Direction.up;
	}
	...
	protected Direction getPrevDirection(final boolean ltr) {
		return horizontalSlide ? ltr ? Direction.left : Direction.right
				: Direction.down;
	}
	...
	protected void updateWeekHeaders() {
		...
		for (int i = 1; i <= 7; i++) {
			...
			if (i < 7) {
				weekHeaders.add(new WebSeparator(WebSeparator.VERTICAL),
						((i - 1) * 2 + 1) + ",0");
			}
		}
	 ...
	 *            date to select and display
	 * @param animate
	 *            whether should animate month transition or not
	 */
	 ...
	 * 
	 * @return true if should perform horizontal slide animation, false
	 *         otherwise
	 */
	...
	public void fireDateSelected(final Date date) {
		for (final DateSelectionListener listener : CollectionUtils
				.copy(dateSelectionListeners)) {
