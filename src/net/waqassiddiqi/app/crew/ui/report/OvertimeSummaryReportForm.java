...
import net.waqassiddiqi.app.crew.report.OvertimeSummaryReport;
...
public class OvertimeSummaryReportForm extends BaseForm {
	...
	private OvertimeSummaryReport overtimeSummaryReport;
	...
	public OvertimeSummaryReportForm(MainFrame owner) {
		super(owner);
	...
	public void setupToolBar() {
		getToolbar().add(new WebLabel("Overtime Summary Report") {{ setDrawShade(true); setMargin(10); }});
	...
	private String generateReport(Vessel vessel, int month, int year) {
		...
		overtimeSummaryReport = new OvertimeSummaryReport(year, month);
		overtimeSummaryReport.generateReport();		
		
		localVelocityContext.put("lstSummary", overtimeSummaryReport.getOvertimeSummaryList());	    
	    localVelocityContext.put("host", overtimeSummaryReport);
	    ...
	    Template reportTemplate = ve.getTemplate("resource/template/over_time_summary.html");
	...
	private void saveAsPdf() {
		fileChooser = new  WebFileChooser() {
			public void approveSelection() {
				...
				OvertimeSummaryReportForm.this.saveFileTarget = getSelectedFile();
				
				if (!FilesUtil.getExtension(OvertimeSummaryReportForm.this.saveFileTarget).equalsIgnoreCase(".pdf")) {					
					SwingWorker<String, Void> worker = new SwingWorker<String, Void>() {

						@Override
						protected String doInBackground() throws Exception {
							return generatePdf(OvertimeSummaryReportForm.this.saveFileTarget.toString());
						}
