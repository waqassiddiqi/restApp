...
import net.waqassiddiqi.app.crew.report.MonthlyOvertimeRestReport;
...
public class MonthlyOvertimeRestReportForm extends BaseForm {
	...
	private MonthlyOvertimeRestReport monthlyOvertimeRestReport;
	...
	public MonthlyOvertimeRestReportForm(MainFrame owner) {
		super(owner);
	...
	public void setupToolBar() {
		getToolbar().add(new WebLabel("Monthly Overtime / Rest Report") {{ setDrawShade(true); setMargin(10); }});
	...
	private String generateReport(Crew crew, Vessel vessel, int month, int year) {
		...
		monthlyOvertimeRestReport = new MonthlyOvertimeRestReport(crew, vessel, month, year);
		monthlyOvertimeRestReport.generateReport();		
		
		localVelocityContext.put("lstEntryTimes", monthlyOvertimeRestReport.getEntryTimeList());	    
	    localVelocityContext.put("host", monthlyOvertimeRestReport);
	    ...
	    Template reportTemplate = ve.getTemplate("resource/template/monthly_overtime_rest_report.html");
	...
	private void saveAsPdf() {
		fileChooser = new  WebFileChooser() {
			public void approveSelection() {
				...
				MonthlyOvertimeRestReportForm.this.saveFileTarget = getSelectedFile();
				
				if (!FilesUtil.getExtension(MonthlyOvertimeRestReportForm.this.saveFileTarget).equalsIgnoreCase(".pdf")) {					
					SwingWorker<String, Void> worker = new SwingWorker<String, Void>() {

						@Override
						protected String doInBackground() throws Exception {
							return generatePdf(MonthlyOvertimeRestReportForm.this.saveFileTarget.toString());
						}
